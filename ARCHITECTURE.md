...
LiveBytes 参考 Linux 目录结构，在系统根目录下创建固定名称的目录，并将工作空间放在 `var/` 下：
...
LiveBytes 通过明确的角色分工来保持系统简单可扩展：

1. **用户**：通过工作空间提交任务、查看结果与管理配置，不直接与内部进程通信。
2. **Agent 核心进程**：负责 LiveBytes 推理思考流、任务解析、状态流转与扩展任务调度。
3. **Agent 扩展任务进程**：面向具体能力的执行单元，可由插件或外部服务实现，专注处理子任务并产出结果。

### 文件系统作为统一交互面

所有角色都仅通过文件系统约定进行交互：

- 用户仅需要操作 `input/`、`output/`、`config/` 等公开目录。
- 核心进程负责维护 `processing/`、`completed/`、`failed/` 等状态目录并为扩展任务生成标准化的任务资料。
- 扩展任务进程读取核心进程准备的任务文件，并输出结果文件以回传执行状态。

这种设计确保不同角色可以解耦演进，同时保持统一的数据面与审计能力。
...
1. 用户在 `input/` 目录创建任务文件（`.lbtask` 后缀）
2. 文件系统监控器检测到新文件
3. 任务被解析并加入处理队列
